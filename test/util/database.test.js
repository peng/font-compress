...
test('create database method test', () => {
  ...
  createDatabase('mytest').then(res => {
  ...
  createDatabase('mytesterr').then(res => {
    expect(res).toEqual({
      result: {},
      fields: undefined
    });
...
test('select database method', () => {
  ...
  selectDatabase('mytest').then(res => {
  ...
  selectDatabase('mytesterr').then(res => {
    expect(res).toEqual({
      result: {},
      fields: undefined
    });
...
test('create database table', () => {
  // success test
  createTable('test_table', ['id SMALLINT', 'account VARCHAR(255)']).then(res => {
    expect(res.result).toEqual({});
  }, err => {});
  // fail test
  createTable('test_table', ['id SMALLINT', 'account VARCHAR(255)', 'err']).then(res => {}, err => {
    expect(err).toEqual({});
  });
});
...
test('insert data method', () => {
  ...
  insertData('test_table', ['id', 'account'], [0, "hahaha"]).then(res => {}, err => {
    expect(err).toEqual({});
  });
});

test('delete data method', () => {
  deleteData('DELETE FROM test_table WHERE id=2').then(res => {
    expect(res.result).toEqual({});
  }, err => {});
  deleteData('DELETE FROM test_tableerr').then(res => {}, err => {
    expect(err).toEqual({});
  })
});
...
test('select data method', () => {
  ...
  selectData('test_table', ['id'], 'id=7err').then(res => {}, err => {
    expect(err).toEqual({});
  })
})
...
test('update data method', () => {
  ...
  updateData('test_table', ["id=22", "account='hahahaha'"], 'id=4err').then(res => {}, err => {
    expect(err).toEqual({});
  })
})